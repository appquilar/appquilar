...
const CategoryLandscapeUpload = ({ landscapeId, isUploading, onSelectFile, onRemove }: Props) => {
    const { profilePicture } = useProfilePicture(landscapeId);
    ...
    const handleFile = async (f: File | null) => {
        ...
        // no revocamos aquí: lo puedes revocar si quieres en un cleanup,
        // pero como pronto llegará el blob del servidor, se reemplazará.
    };
    ...
    return (
        <div className="space-y-2">
            <p className="text-sm font-medium">Imagen principal / cabecera</p>
            ...
            >
                {previewUrl ? (
                    <>
                        <img src={previewUrl} alt="header" className="w-full h-full object-cover" />
            ...
            <p className="text-xs text-muted-foreground">
                {isUploading ? "Subiendo..." : "Se guarda como ID (landscape_image_id)."}
            </p>
        </div>
    );
};

export default CategoryLandscapeUpload;
